...
export const LeftSideProblemDescription = ({
  ...
  response: [ResponseProblem, ResponseTestCases[], ResponseExample[]];
}) => {
  const problem = response[0];
  const examples = response[2];
        ...
        >
          ...
          <div className="mt-[28px] pb-[28px] px-[28px]">
            ...
            <div className="description mt-[21px] space-y-[18px]">
              ...
              <p className="d2 text-white text-[20px] ">{problem.inputText2}</p>
              <p className="d3 text-white text-[20px]">{problem.inputText3}</p>
            </div>
            <div>
...
export const RightSideCodeEditor = ({
  ...
  const testCases = response[1].map((testCase) =>
    JSON.stringify(testCase.input || testCase.input, null, 2)
  );

  const targetCases = response[1].map((testCase) =>
    JSON.stringify(testCase.input.target || testCase.output.answer, null, 2)
  );
  ...
  const [userCode, setUserCode] = useState(response[0].starterFunctionName);
...
px-3 rounded-b-none items-center "
   ...
   rounded-b-none  "
          ...
          <CodeMirror
            value={response[0].starterFunctionName}
              ...
              <div className="py-[28px] px-[48px]  ">
                <div className="flex items-center gap-3">
                  ...
                  <p
                    onClick={(e) => {
                      const spanValue =
                        (e.target as HTMLSpanElement).textContent?.split("") ||
                        [];
                      //   setCaseShow(+spanValue[5]);
                      handleCaseChange(Number(spanValue[5]));
                    }}
                    className="caseclass"
                  >
                    {" "}
                    <span>Case {caseCount} </span>
                  </p>{" "}
                  <p
                    onClick={(e) => {
                      const spanValue =
